...
namespace Glitch.Functional
{
    public abstract partial record Result<TOkay, TError>
    {
        ...
        public OneOf<TOkay, TError> AsLeft() => LeftOrElse(x => x);

        public OneOf<TError, TOkay> AsRight() => RightOrElse(x => x);

        public OneOf<TOkay, TRight> LeftOr<TRight>(TRight value) => LeftOrElse(_ => value);

        public OneOf<TOkay, TRight> LeftOrElse<TRight>(Func<TError, TRight> func)
            => Match(OneOf<TOkay, TRight>.Left, func.Then(OneOf<TOkay, TRight>.Right));

        public OneOf<TLeft, TOkay> RightOr<TLeft>(TLeft value) => RightOrElse(_ => value);

        public OneOf<TLeft, TOkay> RightOrElse<TLeft>(Func<TError, TLeft> func)
            => Match(OneOf<TLeft, TOkay>.Right, func.Then(OneOf<TLeft, TOkay>.Left));
