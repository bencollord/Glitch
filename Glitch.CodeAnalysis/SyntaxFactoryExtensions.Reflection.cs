...
public static partial class SyntaxFactoryExtensions
{
    ...
    {
        public static TypeSyntax TypeName(Type type) => SyntaxFactory.ParseTypeName(type.Signature());
        ...
        public static BaseTypeDeclarationSyntax TypeDeclaration(Type type)
        {
                ...
                return SyntaxFactory.EnumDeclaration(type.Name)
                    ...
                    .WithMembers(
                        SeparatedList(
                            ...
                            fields.Select(f => SyntaxFactory.EnumMemberDeclaration(f.Name))));
            }
            ...
            TypeDeclarationSyntax declaration = type.IsValueType ? SyntaxFactory.StructDeclaration(type.Name) : SyntaxFactory.ClassDeclaration(type.Name);
        ...
        public static FieldDeclarationSyntax FieldDeclaration(FieldInfo field)
        {
            ...
            return SyntaxFactory.FieldDeclaration(
                VariableDeclaration(
                    SyntaxFactory.TypeName(field.FieldType),
                    field.Name))
                .WithModifiers(modifiers.ToTokenList());
        }
        ...
        public static PropertyDeclarationSyntax PropertyDeclaration(PropertyInfo property)
        {
            ...
            var getAccessor = getMethod.Select(_ => SyntaxFactory.AccessorDeclaration(SyntaxKind.GetAccessorDeclaration));
            var setAccessor = setMethod.Select(_ => SyntaxFactory.AccessorDeclaration(SyntaxKind.SetAccessorDeclaration))
            ...
            return SyntaxFactory.PropertyDeclaration(TypeName(property.PropertyType), property.Name)
                .WithModifiers(modifiers.ToTokenList())
                .WithAccessorList(
        ...
        public static ConstructorDeclarationSyntax ConstructorDeclaration(ConstructorInfo constructor)
        {
            ...
            var identifier = SyntaxFactory.Identifier(constructor.DeclaringType.Name);

            var parameters = constructor.GetParameters()
                .Select(p => new
                {
                    Name = SyntaxFactory.Identifier(p.Name ?? $"p{p.Position}"),
                    Type = SyntaxFactory.ParseTypeName(p.ParameterType.Signature()),
                    // TODO Default values
                });
            ...
            return SyntaxFactory.ConstructorDeclaration(identifier)
                ...
                .WithParameterList(
                    SyntaxFactory.ParameterList(
                        SeparatedList(SyntaxFactory.Token(SyntaxKind.CommaToken), // TODO Parameter modifiers
                            parameters.Select(p =>
        ...
        public static MethodDeclarationSyntax MethodDeclaration(MethodInfo method)
        {
            ...
            // TODO Override
            var returnType = SyntaxFactory.ParseTypeName(method.ReturnType.Signature());
            ...
            var parameters = method.GetParameters()
                ...
                .Select(p => Parameter(
                    p.Type.WithTrailingTrivia(SyntaxFactory.Space),
                    p.Name));

            var parameterList = parameters.Any() 
                ? SeparatedList(
                    SyntaxFactory.Token(SyntaxKind.CommaToken)
                        .WithTrailingTrivia(SyntaxFactory.Space), 
                    parameters) 
