...
public static partial class Effect
{
    ...
    public static Effect<T> Lift<T>(Func<Result<T, Error>> function) => Lift(() => function().Match(Result.Okay, Result.Fail<T>));
    ...
    public static Effect<T> Try<T>(Func<Result<T, Error>> function) => Lift(() => function().Match(Result.Okay, Result.Fail<T>));
