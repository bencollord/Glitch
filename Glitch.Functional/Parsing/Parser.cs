...
namespace Glitch.Functional.Parsing
{
    ...
    public partial class Parser<TToken, T>
    {
        ...
        public virtual Parser<TToken, Nothing> Not() => new(input =>
        {
            var result = parser(input);

            return result.WasSuccessful
                 ? ParseResult.Error<TToken, Nothing>("Negated parser was successful", input)
                 : ParseResult.Okay(Nothing.Value, result.Remaining);
        });
        ...
        public static Parser<TToken, T> operator >>(Parser<TToken, T> x, Parser<TToken, Nothing> y) => x.Then(v => y.Map(_ => v));
